...
var moment_1 = require("../../../../assets/js/moment");
var data_table;
...
function initializeTable() {
    document.getElementById("footer-content").innerHTML = "Date Created:  " + moment_1["default"]().format('YYYY-MM-DD:h:m:s') + " \n                  BHT-Core Version : " + sessionStorage.coreVersion + " \n                  ART Version : " + sessionStorage.ARTVersion + " \n                  API Version " + sessionStorage.apiVersion;
...
function assignTD(tds, num, gender, age) {
    ...
    // console.log(gender);
    if (gender === "Male") {
        num.tx_new.forEach(function (element) {
            clients.tx_new.push(element);
        });
        num.tx_curr.forEach(function (element) {
...
function parsePatient(results) {
    ...
    var arv_number = results.patient_identifiers.filter(function (el) {
        return el.identifier_type === 4 ? el.identifier : '';
    });
    results.person.addresses.filter(function (el) {
        return;
    });
    try {
...
function fetchRegimens(row) {
    ...
    var dates = quarterEndDate(quarter);
    var url = sessionStorage.apiProtocol + "://" + sessionStorage.apiURL + ":" + sessionStorage.apiPort + "/api/v1";
    url += "/disaggregated_regimen_distribution";
    url += "?date=" + moment_1["default"]().format('YYYY-MM-DD');
...
function addRegimen(data, tds) {
    ...
    for (var regimen in data) {
        ...
        totalIDs = __spreadArrays(data[regimen]);
    }
